...
if check_namespace "financial-inference"; then
    pass "Application namespace 'financial-inference' exists"
else
    fail "Application namespace 'financial-inference' missing"
fi
...
LOCAL_SCHEDULER_REPLICAS=$(kubectl get sts seldon-scheduler -n financial-inference -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
if [[ "$LOCAL_SCHEDULER_REPLICAS" -gt 0 ]]; then
    pass "Local scheduler in financial-inference is running ($LOCAL_SCHEDULER_REPLICAS replicas) - Scoped Operator Pattern"
else
    fail "Local scheduler in financial-inference is not running - required for Scoped Operator Pattern"
fi
...
MLSERVER_REPLICAS=$(kubectl get sts mlserver -n financial-inference -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
...
MLSERVER_CPU_REQUEST=$(kubectl get sts mlserver -n financial-inference -o jsonpath='{.spec.template.spec.containers[?(@.name=="mlserver")].resources.requests.cpu}' 2>/dev/null || echo "unknown")
MLSERVER_MEMORY_REQUEST=$(kubectl get sts mlserver -n financial-inference -o jsonpath='{.spec.template.spec.containers[?(@.name=="mlserver")].resources.requests.memory}' 2>/dev/null || echo "unknown")
...
if kubectl exec -n financial-inference sts/mlserver -c agent -- \
    nslookup seldon-scheduler.financial-inference.svc.cluster.local >/dev/null 2>&1; then
...
AGENT_SCHEDULER_LOGS=$(kubectl logs -n financial-inference sts/mlserver -c agent --tail=50 2>/dev/null | grep -i "subscribed to scheduler" | tail -1)
...
if check_pod_ready "financial-inference" "app=seldon-envoy"; then
...
ENVOY_XDS_ERRORS=$(kubectl logs -n financial-inference deployment/seldon-envoy --tail=20 2>/dev/null | grep -c "upstream connect error" || echo "0")
...
ENVOY_SCHEDULER_HOST=$(kubectl exec -n financial-inference deployment/seldon-envoy -- env | grep SELDON_SCHEDULER_SERVICE_HOST | cut -d'=' -f2 2>/dev/null || echo "unknown")
LOCAL_SCHEDULER_IP=$(kubectl get svc seldon-scheduler -n financial-inference -o jsonpath='{.spec.clusterIP}' 2>/dev/null || echo "unknown")
...
BASELINE_STATUS=$(kubectl get model baseline-predictor -n financial-inference -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "unknown")
...
ENHANCED_STATUS=$(kubectl get model enhanced-predictor -n financial-inference -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "unknown")
...
# Check experiment
EXPERIMENT_STATUS=$(kubectl get experiment financial-ab-test-experiment -n financial-inference -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "unknown")
...
if kubectl get networkpolicy -n financial-inference >/dev/null 2>&1; then
    POLICY_COUNT=$(kubectl get networkpolicy -n financial-inference --no-headers | wc -l)
    pass "Network policies exist in financial-inference namespace ($POLICY_COUNT policies)"
    
    # Check DNS egress
    DNS_EGRESS=$(kubectl get networkpolicy -n financial-inference -o yaml | grep -A5 -B5 "port: 53" | grep -c "port: 53" || echo "0")
    ...
    warn "No network policies found in financial-inference namespace"
fi
...
MLSERVER_CPU_USAGE=$(kubectl top pods -n financial-inference --no-headers | grep mlserver | awk '{print $2}' | head -1)
MLSERVER_MEMORY_USAGE=$(kubectl top pods -n financial-inference --no-headers | grep mlserver | awk '{print $3}' | head -1)

info "MLServer resource usage: CPU=$MLSERVER_CPU_USAGE, Memory=$MLSERVER_MEMORY_USAGE"

# Check if HPA exists
if kubectl get hpa -n financial-inference >/dev/null 2>&1; then
    HPA_COUNT=$(kubectl get hpa -n financial-inference --no-headers | wc -l)
