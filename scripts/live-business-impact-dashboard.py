...
class BusinessMetricsCalculator:
    ...
    async def get_real_business_data(self, mlflow_db, prometheus_session) -> Dict:
        """Extract real business metrics from production systems"""
        business_data = {}
        
        # Get A/B test results from MLflow
        if mlflow_db:
            business_data.update(await self._get_mlflow_business_metrics(mlflow_db))
    ...
    async def _get_mlflow_business_metrics(self, mlflow_db) -> Dict:
        """Get business-relevant metrics from MLflow experiments"""
        query = """
        SELECT 
        ...
        AND (m.key IN ('accuracy', 'precision', 'recall', 'f1_score', 'mse', 'mae', 'r2_score')
             OR p.key IN ('model_type', 'model_variant', 'test_size', 'random_state'))
        ...
        try:
            ...
            # Process into business metrics
            metrics = {}
            
            # Group by run to get complete model performance
            for run_name in df['run_name'].unique():
                ...
                for _, row in run_data.iterrows():
                    if not pd.isna(row['metric_name']):
                        run_metrics[row['metric_name']] = float(row['metric_value'])
                    if not pd.isna(row['param_name']):
                        run_params[row['param_name']] = row['param_value']
                
                if run_metrics:
                    metrics[run_name] = {
                        'metrics': run_metrics,
                        'params': run_params,
                        'timestamp': run_data['timestamp'].max()
                    }
            
            print(f"📊 Retrieved {len(metrics)} model runs from MLflow")
            return {'mlflow_models': metrics}
            
        except Exception as e:
            print(f"❌ Error querying MLflow for business metrics: {e}")
            return {}
...
async def generate_live_business_dashboard(config: Dict):
    ...
    try:
        ...
        mlflow_db = psycopg2.connect(
            ...
            database=config['mlflow']['database'],
            user=config['mlflow']['username'],
    ...
    business_data = await calculator.get_real_business_data(mlflow_db, prometheus_session)
...
async def main():
    ...
    config = {
        'mlflow': {
            ...
            'username': os.getenv('MLFLOW_DB_USER', 'mlflow'),
