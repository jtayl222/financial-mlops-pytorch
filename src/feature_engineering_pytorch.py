...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="feature_engineering_pytorch"):
        ...
        # --- Create Sequences Following Shape Contract ---
        def create_sequences(features, targets, sequence_length):
            """Create sequences following shape contract: (n_sequences, sequence_length, n_features)"""
            sequences = []
            sequence_targets = []
            
            for i in range(len(features) - sequence_length + 1):
                seq = features[i:i + sequence_length]  # Shape: (sequence_length, n_features)
                target = targets[i + sequence_length - 1]  # Target for last timestep
        ...
        train_sequences, train_seq_targets = create_sequences(
            train_features_scaled, train_target_series.values, SEQUENCE_LENGTH
        )
        val_sequences, val_seq_targets = create_sequences(
            val_features_scaled, val_target_series.values, SEQUENCE_LENGTH
        )
        test_sequences, test_seq_targets = create_sequences(
            test_features_scaled, test_target_series.values, SEQUENCE_LENGTH
        )

        logging.info(f"Created sequences following shape contract:")
        logging.info(f"  Train sequences: {train_sequences.shape} targets: {train_seq_targets.shape}")
        logging.info(f"  Val sequences: {val_sequences.shape} targets: {val_seq_targets.shape}")
        logging.info(f"  Test sequences: {test_sequences.shape} targets: {test_seq_targets.shape}")

        # Save both time series and sequences (following shape contract)
        # Time series data (for reference and debugging)
        ...
        np.save(train_features_path, train_features_scaled)
        np.save(train_targets_path, train_target_series.values)
        np.save(val_features_path, val_features_scaled)
        np.save(val_targets_path, val_target_series.values)
        np.save(test_features_path, test_features_scaled)
        np.save(test_targets_path, test_target_series.values)
        
        # Sequence data (for model training - shape contract compliant)
        train_seq_path = os.path.join(PROCESSED_DATA_DIR, 'train_sequences.npy')
        train_seq_targets_path = os.path.join(PROCESSED_DATA_DIR, 'train_sequence_targets.npy')
        val_seq_path = os.path.join(PROCESSED_DATA_DIR, 'val_sequences.npy')
        val_seq_targets_path = os.path.join(PROCESSED_DATA_DIR, 'val_sequence_targets.npy')
        test_seq_path = os.path.join(PROCESSED_DATA_DIR, 'test_sequences.npy')
        test_seq_targets_path = os.path.join(PROCESSED_DATA_DIR, 'test_sequence_targets.npy')

        np.save(train_seq_path, train_sequences)
        np.save(train_seq_targets_path, train_seq_targets)
        np.save(val_seq_path, val_sequences)
        np.save(val_seq_targets_path, val_seq_targets)
        np.save(test_seq_path, test_sequences)
        np.save(test_seq_targets_path, test_seq_targets)
        ...
        # Log sequence data as artifacts
        mlflow.log_artifact(train_seq_path, artifact_path="sequences")
        mlflow.log_artifact(train_seq_targets_path, artifact_path="sequences")
        mlflow.log_artifact(val_seq_path, artifact_path="sequences")
        mlflow.log_artifact(val_seq_targets_path, artifact_path="sequences")
        mlflow.log_artifact(test_seq_path, artifact_path="sequences")
        mlflow.log_artifact(test_seq_targets_path, artifact_path="sequences")
        
        logging.info("Processed data and sequences saved and logged to MLflow.")

        # Log file existence and shapes for debugging
        for path, arr in [
            (train_features_path, train_features_scaled),
            (train_targets_path, train_target_series.values),
            (val_features_path, val_features_scaled),
            (val_targets_path, val_target_series.values),
            (test_features_path, test_features_scaled),
            (test_targets_path, test_target_series.values),
            (train_seq_path, train_sequences),
            (train_seq_targets_path, train_seq_targets),
            (val_seq_path, val_sequences),
            (val_seq_targets_path, val_seq_targets),
            (test_seq_path, test_sequences),
            (test_seq_targets_path, test_seq_targets),
        ]:
        ...
        # Log metrics following shape contract
        mlflow.log_metric("num_train_sequences", len(train_sequences))
        mlflow.log_metric("num_val_sequences", len(val_sequences))
        mlflow.log_metric("num_test_sequences", len(test_sequences))
        ...
        shape_contract = {
            ...
            "input_shape": [SEQUENCE_LENGTH, train_features_scaled.shape[1]],
            "n_timestamps": len(train_features_scaled),
            "n_train_sequences": len(train_sequences),
            "n_val_sequences": len(val_sequences),
            "n_test_sequences": len(test_sequences),
            "target_type": "binary_classification",
            "pipeline_version": "1.0_shape_contract_compliant"
        }
        
        # Save shape contract metadata
        import json
        ...
        logging.info("Feature engineering completed following shape contract!")
        logging.info(f"Shape contract: input_shape={shape_contract['input_shape']}, sequences created for consistent A/B testing")
